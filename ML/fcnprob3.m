...
function [ae1, e1] = fcnprob3(input, table, d1, flags)

%geo
Ci=numel(d1.z.ge);  zv=zeros(1,Ci);
ae=d1.aepdf;  aecrust=zv;  aemantle=zv;
e=d1.epdf;  ecrust=zv;  emantle=zv;
if (Ci<5000 || flags.status.ML2) && Ci>0
    r2=fcnindex1(table.mev.e, d1.z.ge, '*exact');  r1=floor(r2);  r3=ceil(r2);
    f=(r2-r1)';
    i1=e.crust(:,r1);  ecrust=i1+f.*(e.crust(:,r3)-i1);
    i1=e.mantle(:,r1);  emantle=i1+f.*(e.mantle(:,r3)-i1);
    if flags.status.ML3
        f=ones(table.udx.n,1)*f;
        apdf=d1.snr.apdf( fcnindex1(d1.snr.ct, d1.mantle.udxecef*d1.z.gudxecef') );
        i1=ae.crust(:,r1);  epdf=i1+f.*(ae.crust(:,r3)-i1);  aecrust=sum(epdf.*apdf, 1);
        i1=ae.mantle(:,r1);  epdf=i1+f.*(ae.mantle(:,r3)-i1);  aemantle=sum(epdf.*apdf, 1);
    end
else
    nbatch = 2000;
    nb = ceil(Ci/nbatch); 
    for ib  = 1:nb
        v0 = (nbatch*ib-nbatch+1):min(nbatch*ib,Ci);
...
%reactor
ae1.kr = zv;
r2=fcnindex1(table.mev.e, d1.z.e, '*exact');  r2 = min(max(r2,1),table.mev.ne);  r1=floor(r2);  r3=min(table.mev.ne,ceil(r2));  
f=(r2-r1)';
i1=e.kr(:,r1);  
e1.kr=i1+f.*(e.kr(:,r3)-i1);
e1.kr = interp1(table.mev.e(:),e.kr(:),d1.z.e)';
if flags.status.ML3
    ae1.krapdf=d1.snr.apdf( fcnindex1(d1.snr.ct, d1.kr.udxecef*d1.z.udxecef') );
    i1=ae.kr(:,r1);      epdf = i1 + bsxfun(@times,f,ae.kr(:,r3)-i1);
    ae1.kr=sum(epdf.*ae1.krapdf, 1);
end

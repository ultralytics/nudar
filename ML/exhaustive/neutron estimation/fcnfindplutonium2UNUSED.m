...
clc; close all
%https://www-pub.iaea.org/MTCD/publications/PDF/P1433_CD/datasets/abstracts/sm_en-20.html
...
h=fig(2,2,2);
axes(h(1))
plot(S.epdfy,'b'); hold on; plot(B.epdfy,'r')
axes(h(2))
x1 = linspace(0,10,30);
...
function z = fcngetcandidates(MC)
    ...
    %a(:,10) = all(abs(MC.xhat(:,1:3,i)) < (input.cube.L-10), 2); %P1 wall cut
    %a(:,11) = all(abs(MC.xhat(:,7:9,i)) < (input.cube.L-10), 2); %P2 wall cut

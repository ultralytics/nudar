...
LLA = [linspace(28.957,28.957,nd)' linspace(50.837,50.837,nd)' zeros(nd,1)]; %portugal
x = lla2ecef(LLA);
...
for ii = 1:nd
    ...
    Ci = input.dValid.validevents(ii); %Ci = # candidate measurements
    %Ci = d1.n.all; %CRLB
    
     %ML3-------------------------------------------------------------------
    if Ci>0
        ae1=fcnprob3local(input, table, d1, flags);  d1.est.ae1=ae1;  
...
%     end
    ...
    %ML1-------------------------------------------------------------------
    s = sqrt(Bi*1.025 + d1.est.br1s^2 + d1.est.d1s^2)*6; %count 1 sigma
